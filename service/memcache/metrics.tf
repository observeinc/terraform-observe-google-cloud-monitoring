resource "observe_dataset" "memcache_metrics" {
  count = local.enable_metrics ? 1 : 0

  workspace   = local.datasets.memcache_metrics.workspace
  name        = local.datasets.memcache_metrics.name
  freshness   = local.datasets.memcache_metrics.freshness
  description = local.datasets.memcache_metrics.description

  inputs = {
    "gcp"      = local.base_module.metrics.oid
    "instance" = observe_dataset.memcache_instance.oid
  }
  ...
  stage {
    input    = "gcp"
    pipeline = <<-EOF
        filter resource_type = "memcache_instance"
  ...
  stage {
    pipeline = <<-EOF
       join on(instance_pkey=@instance.instance_pkey), displayName:@instance.displayName, discoveryEndpoint:@instance.discoveryEndpoint
      EOF
  }
  stage {
    pipeline = <<-EOF
...
resource "observe_link" "instance_to_metric" {
  count     = local.enable_metrics ? 1 : 0
  workspace = local.datasets.memcache_metrics.workspace
  source    = one(observe_dataset.memcache_metrics).oid
  target    = observe_dataset.memcache_instance.oid
  fields    = ["instance_pkey"]
  label     = "Memcache Instance"
}
