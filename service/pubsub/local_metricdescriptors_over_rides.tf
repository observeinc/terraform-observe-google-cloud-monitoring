locals {
  ...
  override_keys_service = keys(local.metrics_definitions_overrides_service)
  ...
  merged_metrics_definitions_service = { for key1, value1 in local.service_metrics_definitions : # loop
    key1 => {
      for key2, value2 in value1 :
      key2 => contains(local.override_keys_service, key1) ?                     # check if metric key in override map
      contains(keys(local.metrics_definitions_overrides_service[key1]), key2) ? # check if property in override map for given key
      local.metrics_definitions_overrides_service[key1][key2] : value2          # use if present otherwise use existing
      : value2                                                                  # otherwise use existing
    }
  }
  ...
  metrics_definitions_overrides_service = {
    example = {
      rollup = "rate"
      unit   = "second"
    }
