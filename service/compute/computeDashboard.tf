...
grep -rh "datasetId" --include computeDashboard.tf | sed -e $'s/,/\\\n/g' | sed -e 's/[[:space:]]//g' | sort | uniq | sed -e 's/"datasetId"://g'
grep -rh "workspace" --include computeDashboard.tf | sed -e $'s/,/\\\n/g' | sed -e 's/[[:space:]]//g' | sort | uniq | sed -e 's/"datasetId"://g'

this command ignores lines with sed commands

sed -i '' '/^[[:space:]]*sed/! s:"41144592":"\${local.compute_instance}":g' computeDashboard.tf
sed -i '' '/^[[:space:]]*sed/! s:"41144593":"\${local.compute_metrics}":g' computeDashboard.tf
sed -i '' '/^[[:space:]]*sed/! s/"o:::workspace:41141634"/"\${local.workspace}"/g' computeDashboard.tf
sed -i '' '/^[[:space:]]*sed/! s:"WORKING/Compute Monitoring":"\${local.dashboard_name}":g' computeDashboard.tf
*/
#
locals {
  compute_instance = observe_dataset.compute.id
  compute_metrics  = one(observe_dataset.compute_metrics[*].id)
  workspace        = var.workspace.oid
  dashboard_name   = format(var.name_format, "Monitoring TF")
}
#
#
#
...
# terraform import observe_dashboard.compute_monitoring 41144622
          ...
          steps = [
            {
              ...
              queryPresentation = {}
              renderType        = null
              summary           = "Compute Metrics (41144593)"
              type              = "InputStep"
            },
            {
              datasetQuery = null
