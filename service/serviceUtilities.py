...
def fetch_metric_descriptors(args_in):
    ...
    for response in page_result:
        ...
        def create_domain_specific_properties(val):
            # print(val)
            if "database" in val:
                database_platform = re.search('(?<=database/)([^/]+)', val).group()
        ...
        metric_name = re.search("[^\/]+$", response.name) # pylint: disable=anomalous-backslash-in-string;
        google_metric_path = re.search('(?<=metricDescriptors/)(.+)', response.name)
        ...
        obj_line={}
        obj_line["terraform_variable_name"]= make_name(metric_category, metric_name)
        ...
        obj_line["monitored_resource_types"]=  make_resource(response.monitored_resource_types)
...
def create_terraform(args_in):
    ...
    terraform_output_file = open(args_in.terraform_file_name, "w", encoding="utf8")
    ...
    terraform_output_file = open(args_in.terraform_file_name, "a",  encoding="utf8")
    ...
    print()
    terraform_file_directory = os.path.dirname(os.path.abspath(args.terraform_file_name))
...
-->
This page lists the {service_name} metrics collected by the GCP Integration. You may wish to navigate this page using the Contents located in the right sidebar, or search for a metric of interest.
