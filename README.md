...
| <a name="input_datastream"></a> [datastream](#input\_datastream) | Datastream to derive OTEL resources from. | <pre>object({<br>    oid     = string<br>    dataset = string<br>  })</pre> | n/a | yes |
...
| <a name="input_metrics_poller_interval"></a> [metrics\_poller\_interval](#input\_metrics\_poller\_interval) | How frequently to poll for metrics from Google Cloud Monitoring. | `string` | `"1m0s"` | no |
